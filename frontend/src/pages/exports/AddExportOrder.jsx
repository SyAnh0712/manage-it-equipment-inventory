...
const AddExportOrder = () => {
  ...
  const [details, setDetails] = useState([{ equipment_id: "", quantity: 1 }]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await exportOrderService.createExportOrder({
        department,
        receiver,
        note,
      ...
      console.error(error);
      toast.error(error?.message || "Failed to create export order");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <Row className="mb-4 align-items-center">
        <Col>
          ...
          <p className="text-muted">
            Add a new export order with equipment details.
          </p>
        </Col>
      </Row>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <div className="text-end">
              ...
              <BSButton type="submit" variant="primary" disabled={submitting}>
                {submitting ? "Saving..." : "Create Export Order"}
              </BSButton>
            </div>
          </Form>
        </Card.Body>
